...
2) Squash them into a single commit.
 -Use Command:
    "git rebase -i" 
    Then it opens the editor, select the squash for the 2 files out of three and then click on button rebase.
    It opens editor again for the commit so write the commit and then click commit button. It squashes the multiple commits into the one. 
